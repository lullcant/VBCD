...
def binary_to_pointcloud(tensor):
    """
    将二值化的张量转换为点云 (x, y, z) 坐标的列表。
    输入：tensor 是形状为 [B, C, D, H, W] 的二值化张量
    输出：pointcloud_list 是一个长度为 B 的列表，每个元素是一个点云的坐标数组
    """
    batch_size, _, D, H, W = tensor.shape
    pointcloud_list = []

    # 遍历每个样本
    for b in range(batch_size):
        # 获取当前样本
        binary_mask = tensor[b].view(-1).cpu().numpy()  # 将其展平为一维
        points = np.where(binary_mask > 0)[0]  # 获取非零点的索引
        coords = np.unravel_index(points, (D, H, W))  # 将索引转换为坐标
        
        pointcloud_list.append(np.vstack(coords).T)  # 组合成 (num_points, 3) 的点云

    return pointcloud_list

def compute_assd_and_hausdorff(pred, gt):
    """
    计算点云的 ASSD 和 Hausdorff Distance
    输入：pred 和 gt 为形状为 [B, C, D, H, W] 的二值化张量
    输出：(ASSD, Hausdorff Distance)
    """
    # 转换为点云
    pred_pointcloud = binary_to_pointcloud(pred)
    gt_pointcloud = binary_to_pointcloud(gt)

    total_assd = 0.0
    total_hausdorff = 0.0

    # 遍历每个样本
    for b in range(len(pred_pointcloud)):
        pred_pts = pred_pointcloud[b]
        gt_pts = gt_pointcloud[b]

        # 计算 ASSD：对称的平均表面距离
        dist_pred_to_gt = cdist(pred_pts, gt_pts)
        dist_gt_to_pred = cdist(gt_pts, pred_pts)

        assd = (np.mean(np.min(dist_pred_to_gt, axis=1)) + np.mean(np.min(dist_gt_to_pred, axis=1))) / 2
        total_assd += assd

        # 计算 Hausdorff Distance
        hausdorff = max(np.max(np.min(dist_pred_to_gt, axis=1)), np.max(np.min(dist_gt_to_pred, axis=1)))
        total_hausdorff += hausdorff

    # 计算平均值
    avg_assd = total_assd / len(pred_pointcloud)
    avg_hausdorff = total_hausdorff / len(pred_pointcloud)

    return avg_assd, avg_hausdorff
